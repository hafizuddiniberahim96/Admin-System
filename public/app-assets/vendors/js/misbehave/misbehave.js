(function(global, factory) {
    ...
    var bindSingle = function(combination, callback, action, sequenceName, level) {
        ...
        self.getMatches(info.key, info.modifiers, {
            type: info.action
    ...
    var getSections = function(elem, callback) {
        var sel, range, tempRange, prefix = '',
            selected = '',
            suffix = '';
    ...
    var allCharacters = /./g;
    var removeIfStartsWith = function(s) {
        return function(line) {
            return line.startsWith(s) ? line.slice(s.length) : line;
        };
    };
    ...
    var Editable = function Editable(elem, ref) {
        if (ref === void 0) ref = {};
        var autoIndent = ref.autoIndent;
        if (autoIndent === void 0) autoIndent = true;
        var autoOpen = ref.autoOpen;
        if (autoOpen === void 0) autoOpen = true;
        var autoStrip = ref.autoStrip;
        if (autoStrip === void 0) autoStrip = true;
        var overwrite = ref.overwrite;
        if (overwrite === void 0) overwrite = true;
        var softTabs = ref.softTabs;
        if (softTabs === void 0) softTabs = 2;
        var replaceTab = ref.replaceTab;
        if (replaceTab === void 0) replaceTab = true;
        var pairs = ref.pairs;
        ...
        if (oninput === void 0) oninput = function() {};
        var undoLimit = ref.undoLimit;
        if (undoLimit === void 0) undoLimit = 0;
        ...
        keys.stopCallback = function() {
            return false;
        }; // work without needing to set combokeys class on elements
        keys.bind('mod+z', function() {
            undoMgr.undo();
            return false
        });
        keys.bind('shift+mod+z', function() {
            undoMgr.redo();
            return false
        });
        if (autoIndent) {
            keys.bind('enter', function() {
                return getSections(elem, function(ref) {
        ...
        if (autoStrip) {
            keys.bind('backspace', function() {
                return getSections(elem, function(ref, selection) {
                    var prefix = ref.prefix;
                    var selected = ref.selected;
                    var suffix = ref.suffix;
        ...
        var fnAutoOpen = function(opening, closing) {
            return function() {
                return getSections(elem, function(ref) {
                    var prefix = ref.prefix;
                    var selected = ref.selected;
        ...
        var fnOverwrite = function(closing) {
            return function() {
                return getSections(elem, function(ref, selection) {
                    var prefix = ref.prefix;
                    var selected = ref.selected;
        ...
        pairs.forEach(function(ref) {
            ...
            if (closing) {
                if (autoOpen) {
                    keys.bind(opening, fnAutoOpen(opening, closing));
                }
                if (overwrite) {
                    keys.bind(closing, fnOverwrite(closing));
                }
            } else {
                if (autoOpen && overwrite) {
                    keys.bind(opening, function() {
                        return getSections(elem, function(ref, selection) {
                            var prefix = ref.prefix;
                            var selected = ref.selected;
                            var suffix = ref.suffix;
                    ...
                    if (autoOpen) {
                        keys.bind(opening, fnAutoOpen(opening, opening));
                    }
                    if (overwrite) {
                        keys.bind(opening, fnOverwrite(opening));
                    }
                }
            }
        });
        ...
        editable.inputListener = elem.addEventListener('input', function() {
            return getSections(elem, update);
        });
    ...
    var testAutoStrip = function(pairs, prefix, selected, suffix) {
        ...
        pairs.forEach(function(ref) {
            ...
            if (prefix.slice(-1) === opening && suffix.charAt(0) === closing) {
                result = true;
            }
        });
        return result
    ...
    var Misbehave = (function(Editable$$1) {
        function Misbehave(elem, opts) {
            ...
            if (typeof opts.store === 'undefined') {
                opts.store = store(getSections(elem));
            }
            if (typeof opts.behavior === 'undefined') {
                opts.behavior = StrUtil;
            }
